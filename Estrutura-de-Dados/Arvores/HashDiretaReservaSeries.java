class HashDiretaReserva {
    HashDiretaReservaSeries tabela[];
    int m1, m2, m, reserva;
    ...
    public HashDiretaReserva(int m1, int m2) {
        this.m1 = m1;
        this.m2 = m2;
        this.m = m1 + m2;
        this.tabela = new HashDiretaReservaSeries[this.m];
        for (int i = 0; i < m1; i++) {
            numComparacoes++;
            tabela[i] = null;
        }
    ...
    // recebo o valor asc da string como parâmetro
    ...
    public boolean inserir(HashDiretaReservaSeries elemento) {
        boolean resp = false;
    ...
    // public void mostrar() {
    // for (int i = m1; i < tabela.length; i++) {
    // if (tabela[i] != null) {
    ...
    public boolean pesquisar(String nome) {
        ...
        if (tabela[pos].getNome().compareTo(nome) == 0) {
            ...
            for (int i = 0; i < reserva; i++) {
                numComparacoes++;
                if (tabela[m1 + i].getNome().compareTo(nome) == 0) {
                    resp = true;
                    i = reserva;
                }
            }
...
class HashDiretaReservaSeries {
    ...
    // construtor primário
    public HashDiretaReservaSeries() {
        nome = "";
        formato = "";
        duracao = "";
        paisDeOrigem = "";
    ...
    public HashDiretaReservaSeries(String nome, String formato, String duracao, String paisDeOrigem,
    ...
    public static HashDiretaReservaSeries lerAtributo(String endereco) {
        HashDiretaReservaSeries s1 = new HashDiretaReservaSeries();
    ...
    public void imprimir(HashDiretaReservaSeries s1) {
    ...
    public static void main(String args[]) throws Exception {
        HashDiretaReserva hashDiretaReserva = new HashDiretaReserva();
        ...
        while (!isFim(entrada)) {
            ...
            hashDiretaReserva.inserir(lerAtributo(caminhoDoArquivo));

            entrada = MyIO.readLine();
        }
        // hashDiretaReserva.mostrar();
        entrada = MyIO.readLine();
        // pesquisando no array
        while (!isFim(entrada)) {
            System.out.println(hashDiretaReserva.pesquisar(entrada) ? " SIM" : " NAO");

            entrada = MyIO.readLine();
        }
        Long tempoFinal = System.currentTimeMillis();
        arquivoLog(tempoFinal - tempoInicial, hashDiretaReserva.numComparacoes);
